var retailApp = angular.module('retailApp', ['ngRoute', 'ngAnimate', 'ngLodash']);
            ...
            Session.searchInventoryUpc(success.text).then(function (response){
              if(response.toString().substring(0,5) != "error"){
                 $rootScope.product = lodash.map(response, function (obj) {
                  ...
                  console.log('Stores', $rootScope.product);
                 }
                else {
              ...
              if(item.rememberMe == true) {
               ...
               Session.login(item.username, item.password).then(function (response) {
                  ...
                  if(response[0]['sessionId'] != null && response[0]['sessionId'] != ""){
                    ...
                    $scope.pageLoading = false;
                    $scope.errorMsg = "Error: Wrong username or password";
                    $scope.serverError = true;
                    $timeout( function(){
                        $scope.serverError = false;
                    }, 5000 );
...
retailApp.controller('homeController', ['$scope', '$location', '$routeParams', '$http', '$timeout', '$window', '$rootScope', 'Session', 'lodash', function($scope, $location, $routeParams, $http, $timeout, $window, $rootScope, Session, lodash) {
    ...
    $scope.scanBarcode = function(){
      ...
      cordova.plugins.barcodeScanner.scan(
        function (success) {
          console.log('Success text', success.text);
          if(success.cancelled != true) {
...
retailApp.controller('cartCheckoutController', ['$scope', '$location', '$routeParams', '$http', '$timeout', '$window', '$rootScope', 'Session', 'Server', 'lodash', function($scope, $location, $routeParams, $http, $timeout, $window, $rootScope, Session, Server, lodash) {
    ...
    Session.getCartItems(sessionId).then(function (response){
        ...
        //$scope.defaultShippingValue = $scope.shippingOptions;
         $timeout( function(){ $scope.pageLoading = false; }, 500 );
...
//    function submitOrder(shipVia, purchaseOrderNumber, dropship, contactName, address1, address2, city, state, zip) {
      Server.submitOrder(shipVia, purchaseOrder, dropship, contactName, address1, address2, city, state, zip).then(function (response){
            //Send Google Anayltics Event Tracking
      ...
      // if(item.dropship) {
      //   if(item.name != null && item.address != null && item.city != null && selectedState != null && item.zip != null){
      //       //process order
      //       if(item.purchaseOrder != null) {
      //         if(item.purchaseOrder.length == 25) {
      //           //send valid purchase order 
      //         } else {
      ...
      //       $scope.pageLoading = true;
      //       localStorage.setItem("shipVia", item.shipVia);
      //       Server.submitOrder(item.shipVia, item.purchaseOrder, "no").then(function (response){
...
retailApp.controller('invoicesController', ['$scope', '$location', '$routeParams', '$http', '$timeout', '$window', '$rootScope', 'Session', 'Server', 'lodash', function($scope, $location, $routeParams, $http, $timeout, $window, $rootScope, Session, Server, lodash) {
      ...
      $scope.submitPayment = function(item, selectedState){
        ...
        if(item.cardName){
          ...
          Server.payInvoice($scope.invoiceIds, $scope.invoiceAmount, item.address1, address2, item.city, item.state, item.zip, cardName, cardNumber, cardExp, cardCsv, $scope.invoices[0].batchId, routingNumber, accountNumber).then(function (response){
            ...
            if($scope.confirmationNumber[0].success == 1){
              ...
              console.log("Confirmation #: ", $scope.confirmationNumber);
            }
        });
      }
...
retailApp.controller('selectStore', ['$scope', '$location', '$routeParams', '$http', '$timeout', '$window', '$rootScope', 'Session', 'lodash', function($scope, $location, $routeParams, $http, $timeout, $window, $rootScope, Session, lodash) {
...
retailApp.factory('Server', ['$http', function ServerFactory($http) {
      ...
      function getCart(sessionId) {
        ...
        if(localStorage.getItem("store")){
          ...
          return $http({
              ...
              data: '{"Columns":[{"Name": "SessionID","Type":"String"},{"Name":"ApplicationName","Type":"String"},{"Name":"Store","Type":"String"}],"Rows":[["' + sessionId + '","' + applicationName + '","' + store + '"]]}',
              method: 'POST'
          }).then(
               function successFn(response) {
                   return response.data;
      ...
      function payInvoice(invoice, amount, address, address2, city, state, zip, cardName, cardNumber, expDate, csv, batchId, routingNumber, accountNumber) {
        var applicationName = "SPI_APP";
        //var tempProduct = JSON.stringify(tempProduct);
        var storeId = "";
        var sessionId = localStorage.getItem("sessionId");
        ...
        if(routingNumber == ""){
          ...
          tempData = '{"Columns":[{"Name": "SessionID","Type":"String"},{"Name": "Invoice","Type":"String"},{"Name": "Amount","Type":"String"},{"Name":"BAddress1","Type":"String"},{"Name":"BAddress2","Type":"String"},{"Name":"BCity","Type":"String"},{"Name":"BState","Type":"String"}, {"Name":"BZip","Type":"String"}, {"Name":"RoutingNumber","Type":"String"}, {"Name":"AccountNumber","Type":"String"}, {"Name":"STaxAmt","Type":"String"}, {"Name":"BatchID","Type":"String"}], "Rows":[["' + sessionId + '",' + invoice + ',"' + amount + '","' + address + '","'+ address2 + '","' + city + '","' + state + '","' + zip + '","' + routingNumber + '","' + accountNumber + '","' + taxAmount + '","' + batchId + '"]]}';
        }
      ...
      function submitOrder(shipVia, purchaseOrderNumber, dropship, contactName, address1, address2, city, state, zip) {
        ...
        //if(dropship == "yes"){
          tempData = '{"Columns":[{"Name": "SessionID","Type":"String"},{"Name": "ApplicationName","Type":"String"},{"Name": "Account","Type":"String"},{"Name":"Store","Type":"String"},{"Name":"ShipVia","Type":"String"},{"Name":"Dropship","Type":"String"},{"Name":"Contact_Name","Type":"String"}, {"Name":"Address_1","Type":"String"}, {"Name":"Address_2","Type":"String"}, {"Name":"City","Type":"String"}, {"Name":"State_Code","Type":"String"}, {"Name":"Zip_Code","Type":"String"}, {"Name":"Country_Code","Type":"String"}, {"Name":"CustPurchOrder","Type":"String"}], "Rows":[["' + sessionId + '","' + applicationName + '","' + account + '","' + storeId + '","'+ shipVia + '","' + dropship + '","' + contactName + '","' + address1 + '","' + address2 + '","' + city + '","' + state + '","' + zip + '","' + purchaseOrderNumber + '",""' + ']]}';
        //} else {
...
retailApp.factory('Session', ['$http', 'Server', '$q', function ServerFactory($http, Server, $q) {
  ...
  var getInvoices = function (sessionId) {
    ...
    Server.getInvoices(sessionId).then(function (response) {
        ...
        for (var i = 0; i < tempInvoices.Rows.length; i++) {
